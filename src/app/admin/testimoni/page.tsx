...
const defaultForm: Omit<Item, "id"> & { id?: string } = { name: "", role: "", review: "", rating: 5, avatar: "" };
...
export default function AdminTestimonialsPage() {
  ...
  async function save() {
    if (form.id) {
      await fetch("/api/testimonials", { method: "PUT", headers: headers(), body: JSON.stringify(form) });
    } else {
  ...
  async function remove(id: string) {
    ...
    await fetch("/api/testimonials", { method: "DELETE", headers: headers(), body: JSON.stringify({ id }) });
    load();
  }
  ...
  return (
    <div className="max-w-6xl">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-white tracking-tight">Testimoni</h1>
          <p className="text-sm text-white/60 mt-0.5">Kelola data testimoni penghuni</p>
        </div>
        ...
        <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center p-4">
          <div className="bg-surface rounded-xl w-full max-w-lg shadow-xl border border-white/10">
            <div className="flex items-center justify-between px-6 h-14 border-b border-white/10">
              <h2 className="text-base font-semibold text-white">{form.id ? "Edit Testimoni" : "Tambah Testimoni"}</h2>
            ...
            <div className="p-6 space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-xs font-medium text-white mb-1.5">Peran/Pekerjaan</label>
                  <input value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })}
                    className="w-full h-10 px-3.5 rounded-lg border border-white/10 bg-dark text-white text-sm focus:outline-none focus:ring-2 focus:ring-gold/40 focus:border-gold/50 transition-all" />
                </div>
              </div>
              <div>
                  <label className="block text-xs font-medium text-white mb-1.5">Testimoni</label>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-white mb-1.5">Rating (1-5)</label>
                  <input type="number" min={1} max={5} value={form.rating} onChange={(e) => setForm({ ...form, rating: Number(e.target.value) })}
                    className="w-full h-10 px-3.5 rounded-lg border border-white/10 bg-dark text-white text-sm focus:outline-none focus:ring-2 focus:ring-gold/40 focus:border-gold/50 transition-all" />
                </div>
                <div>
                  <label className="block text-xs font-medium text-white mb-1.5">URL Avatar</label>
      ...
      <div className="space-y-3">
          ...
          <div key={item.id} className="bg-surface rounded-xl border border-white/10 p-5 hover:shadow-lg transition-shadow">
            <div className="flex items-start justify-between">
              <div className="flex items-center gap-3">
                {item.avatar && (
                  <img src={item.avatar} alt="" className="w-10 h-10 rounded-full object-cover ring-2 ring-white/10" />
                )}
                <div>
            ...
            <p className="text-sm text-white/60 mt-3 leading-relaxed">{item.review}</p>
        ...
        {items.length === 0 && (
          <p className="text-center text-white/60 text-sm py-16">Belum ada testimoni</p>
        )}
      </div>
    </div>
  );
